...
const PurchaseProductController = {
  async addPurchaseProduct(
    req: Request<unknown, unknown, PurchaseProduct>,
    res: Response,
    next: NextFunction
  ) {
    try {
  ...
  async getPurchaseProduct(
    req: Request<unknown, unknown, unknown, PurchasedProductQuery>,
    res: Response,
    next: NextFunction
  ) {
    try {
      ...
      const response = await PurchaseProductService.getPurchaseProduct(
        userId,
        query
      );
      return successResponse({
  ...
  async getSalesDataByMonth(req: Request, res: Response, next: NextFunction) {
    try {
      const sellerId = res.locals.user._id;
      const response = await PurchaseProductService.getSalesDataByMonth(
        sellerId
      );
      return successResponse({
        response: res,
        message: "Fetched sold product successfully",
        data: response,
        status: 200,
      });
    } catch (error) {
